...
let allCaps = [];
const firstNameCap = (arrRun) => {
    allCaps.push(`${arrRun.first_name.toUpperCase()}`)
};
runners.map(firstNameCap);
console.log(allCaps);

// ==== Challenge 3: Use .filter() ====
// The large shirts won't be available for the event due to an ordering issue.  Get a list of runners with large sized
// shirts so they can choose a different size. Return an array named largeShirts that contains information about the runners that
...
// Problem 1
//Make a new array with the runners who donated more than 150$.

function goodDonor(arr, cb) {
    return cb(arr.filter(money => money.donation >= 150))
}
goodDonor(runners, function (participant) {
    console.log(participant.map(money => {
...
let alph = [];
const firstNameAlph = (arrRun) => {
    alph.push(`${arrRun.last_name}`)
    alph.sort()
};
...
let greeting = [];
runners.forEach((names => {
    greeting.push(`Hello, my name is ${names.first_name} ${names.last_name}`)
}));
